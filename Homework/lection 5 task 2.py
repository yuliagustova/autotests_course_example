...
def repeats(our_str):
    """
    Принимает на вход строку our_str и подсчитывает, сколько раз символ уже встречался. Добавляет количество к символам
    через "_"
    :param our_str: строка, в которой необходимо подсчитать, сколько раз уже встречался символ
    :return: возвращает строку new_str с добавленным количеством, сколько раз символ уже встречался, к символам
    через "_"
    """
    new_str = ''
    counter = {}
    for letter in our_str:
        ...
        new_str += f'{letter}_{counter[letter]}'
    return new_str
